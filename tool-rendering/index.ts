	...
	pi.registerTool({
		...
		renderCall: (args, theme, context) => {
			if (isSkillPath(args?.path)) {
				...
				text.setText(
					theme.fg("toolTitle", theme.bold("⚡Load skill ")) +
			...
			if (!context.expanded) {
				...
				const lineRange = formatReadLineRange(
					args as Record<string, unknown> | undefined,
				)
				text.setText(
					theme.fg("toolTitle", theme.bold(`Read ${path}`)) +
						theme.fg("dim", lineRange),
				)
				return text
			}

			if (readToolDef.renderCall)
				return readToolDef.renderCall(args, theme, context)
			return (
				(context.lastComponent as Text | undefined) ??
				new Text(
					theme.fg("toolTitle", `read ${String(args?.path ?? "")}`),
					0,
					0,
				)
			)
		...
		renderResult: (result, options, theme, context) => {
			...
			if (readToolDef.renderResult)
				return readToolDef.renderResult(result, options, theme, context)
			const text =
				(context.lastComponent as Text | undefined) ?? new Text("", 0, 0)
		...
		renderCall: (args, theme, context) => {
			...
			text.setText(
				theme.fg("toolTitle", theme.bold(`$ ${command}`)) +
					...
					theme.fg(
						"dim",
						` (${keyHint("app.tools.expand", "to expand output")})`,
					),
			)
			return text
		},
		renderResult: (result, options, theme, context) => {
			...
			if (result.isError) {
				text.setText(
					theme.fg(
						"error",
						`Command failed (${keyHint("app.tools.expand", "to inspect")})`,
					),
				)
				return text
			}
