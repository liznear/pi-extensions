...
async function runSubagent(
	...
	return {
		text: parseSubagentText(result.stdout),
...
export default function agentExtension(pi: ExtensionAPI): void {
	...
	function cyclePrimary(ctx: ExtensionContext): void {
		...
		const idx = Math.max(
			0,
			primaries.findIndex((p) => p.name === activeAgentName),
		)
		...
		handler: async (args, ctx) => {
			...
			if (command === "" || command === "list") {
				const lines = profiles.map(
					(p) =>
						`${p.id} / ${p.name} [${p.type}]${p.allowedTools ? ` tools=${p.allowedTools.join(",")}` : ""}`,
				)
				ctx.ui.notify(
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			agent_name: Type.String({
				description: "Subagent name or id from ~/.pi/agent/agents/*.md",
			}),
			prompt: Type.String({
				description: "Prompt for the subagent (no parent context is included)",
			}),
		}),
		renderCall: (args, theme) => {
			return new Text(
				theme.fg("toolTitle", theme.bold(`run_subagent ${args.agent_name}`)),
				0,
				0,
			)
		},
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			const sub = profiles.find(
				(p) =>
					(p.name === params.agent_name || p.id === params.agent_name) &&
					p.type === "subagent",
			)
			if (!sub) {
				const available =
				...
				return {
					content: [
						{
							type: "text" as const,
							text: `Unknown subagent '${params.agent_name}'. Available: ${available}`,
						},
					],
					isError: true,
					details: {},
			...
			if ((result.code ?? 1) !== 0) {
				return {
					content: [
						{
							type: "text" as const,
							text: `Subagent failed: ${result.stderr || "unknown error"}`,
						},
					],
					isError: true,
					details: {},
			...
			return {
				content: [
					{ type: "text" as const, text: result.text || "(no output)" },
				],
