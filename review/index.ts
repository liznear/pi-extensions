...
const pendingReviews = new Map<
	string,
	{ request: string; cwd: string; createdAt: number }
>()
...
function formatToolCall(
	...
	switch (toolName) {
		case "bash":
			return `$ ${shorten(str(args.command), 100)}`
		case "read":
			return `read ${str(args.path)}`
		case "grep":
			return `grep /${str(args.pattern).replace("...", "")}/ in ${str(args.path).replace("...", ".")}`
		case "find":
			return `find ${str(args.pattern).replace("...", "*")} in ${str(args.path).replace("...", ".")}`
		case "ls":
			return `ls ${str(args.path).replace("...", ".")}`
		default:
			return `${toolName} ${shorten(JSON.stringify(args ?? {}), 80)}`
	}
}
...
export default function reviewExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		description:
			"Run an isolated reviewer subprocess. Internal tool used by /review command.",
			...
			if (!entry) {
				return {
					...
					content: [
						{
							type: "text",
							text: "Unauthorized review invocation (missing/expired nonce). Use /review.",
						},
					],
				}
			}
			...
			try {
					...
					(line) => {
						...
						try {
							...
							if (event.type === "tool_execution_start") {
								lines.push(
									`→ ${formatToolCall(event.toolName ?? "tool", event.args ?? {})}`,
								)
								emit("running")
								return
							}
							...
							if (
								...
								if (
									...
									error =
										...
										`Reviewer ${event.message.stopReason}`
								}
								emit("running")
							}
						} catch {
				...
				if (res.code !== 0) {
					error =
						(res.stderr || "").trim() ||
						error ||
						`Reviewer failed with exit ${res.code}`
					emit("completed")
					return {
						isError: true,
		...
		renderCall(args, theme) {
			return new Text(
				theme.fg("toolTitle", theme.bold("review_subagent ")) +
					theme.fg("muted", `nonce=${String(args.nonce ?? "")}`),
				0,
				0,
			)
		},
		...
		renderResult(result, { expanded, isPartial }, theme) {
			...
			if (!isPartial) {
				return new Text(
					theme.fg("muted", "Review complete. Final report shown below."),
					0,
					0,
				)
			}
	...
	pi.registerCommand("review", {
		description:
			"Run isolated review of uncommitted changes or a custom review scope",
		handler: async (args, ctx) => {
			...
			if (!diff.trim()) {
				...
				ctx.ui.notify(
					"No diff output. Untracked files are not reviewable until added (git add).",
					"warning",
				)
				return
			}
