...
const AskUserParams = Type.Object({
	...
	options: Type.Optional(
		Type.Array(OptionSchema, { description: "Optional list of choices" }),
	),
	allowCustom: Type.Optional(
		Type.Boolean({
			description: "Allow user to type custom answer (default: true)",
		}),
	),
})
...
const QuestionSchema = Type.Object({
	...
	options: Type.Optional(
		Type.Array(OptionSchema, { description: "Optional choices" }),
	),
	allowCustom: Type.Optional(
		Type.Boolean({ description: "Allow custom answer (default: true)" }),
	),
})
	...
	return ctx.ui.custom<{ answers: Answer[]; cancelled: boolean }>(
		(tui, theme, _kb, done) => {
			...
			function currentOptions(): (QuestionOption & { isOther?: boolean })[] {
				...
				if (!q) return []
				const opts: (QuestionOption & { isOther?: boolean })[] = [
					...(q.options || []),
				]
				...
				if (inputMode && q) {
					...
					for (const question of questions) {
						...
						if (answer) {
							...
							add(
								`${theme.fg("muted", ` ${question.id}: `)}${theme.fg("text", prefix + answer.answer)}`,
							)
						}
					}
					lines.push("")
...
export default function askUser(pi: ExtensionAPI) {
			...
			if (options.length === 0) {
				return {
					content: [
						{
							type: "text",
							text: "Error: No options provided and custom input disabled",
						},
					],
					details: {
						question: params.question,
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const answerLines = result.answers.map(
		...
		renderResult(result, _options, theme, _context) {
			...
			const lines = details.answers.map(
				(a) =>
					theme.fg("success", "✓ ") +
					theme.fg("accent", a.questionId) +
					": " +
					(a.wasCustom ? theme.fg("muted", "(wrote) ") : "") +
					a.answer,
			)
